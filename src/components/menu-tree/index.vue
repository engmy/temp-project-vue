<template>
  <div class="menu-tree">
    <el-popover ref="popover" placement="bottom-start" trigger="click" popper-class="popper-menu-tree" width="530px">
      ...
      <el-tree ref="tree" node-key="id" :data="treeList" :props="props" :highlight-current="true"
      ...
      <template #reference>
        <el-input :value="name" readonly placeholder="请选择菜单"></el-input>
      </template>
    </el-popover>
  </div>
</template>
...
export default
{
    ...
    setup(props, context)
    {
        ...
        const initMenuList = () =>
        {
            let propsList = props.menuList;
            propsList.unshift({ name: "一级菜单", id: "0"});
        ...
        return { popover, tree, ...toRefs(state), treeList, name, currentChange, filterNode };
    }
};
</script>
